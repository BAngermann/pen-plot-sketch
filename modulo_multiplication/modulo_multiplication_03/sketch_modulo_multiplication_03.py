...
class ModuloMultiplication01Sketch(vsketch.SketchClass):
    ...
    def draw(self, vsk: vsketch.Vsketch) -> None:
        ...
        intersection_np = np.empty((len(intersections),2))

        for i in range(len(intersections)):
           intersection_np[i,0] = intersections[i].x
           intersection_np[i,1] = intersections[i].y

        vor = Voronoi(intersection_np)
  
        for i in range(np.shape(vor.ridge_vertices)[0]):
           if(vor.ridge_vertices[i][0] != -1 and vor.ridge_vertices[i][1] != -1  ):
            v1 = vor.vertices[vor.ridge_vertices[i][0]]
            v2 = vor.vertices[vor.ridge_vertices[i][1]]
            if (np.linalg.vector_norm(v1) < 2*self.r and np.linalg.vector_norm(v2) < 2*self.r ):
              vsk.line(v1[0],v1[1],v2[0],v2[1])
