...
fn main() {
    ...
    let grid = SquareGrid::new(4,Some(3));
    println!("Chosen solution index: {}", grid.solution_index);
    print_matrix(grid.solution());

    for square in grid.iter_squares() {
        println!(
            "Square top-left ({}, {}), size {},render_pos ({},{}), render_scale {}",
            square.row, square.col, square.size,square.render_pos.0,square.render_pos.1,square.render_scale
        );
    }
}
